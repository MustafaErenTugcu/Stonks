  ...
  ngOnInit(): void {
   ...
   addNewMarketData(newItem) {
    ...
    data.forEach(item => {
      ...
      const changeColor = item.change >= 0 ? 'text-green-600 font-bold' : 'text-red-600 font-bold';
      const changeSymbol = item.change >= 0 ? '▲' : '▼';

      row.innerHTML = `
                    <td class="p-3 text-sm text-center font-medium text-black-500">${item.name}</td>
                    <td class="p-3 text-sm text-center text-black-500">${item.value.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
